...
// lấy dl từ url, form:
if (($album_id = $nv_Request->get_int('album_id', 'get', 0)) > 0) {
    $_SESSION["album_id"] = $album_id;
}
$get_data['album_id'] = $_SESSION["album_id"];
...
$folder_name = '/uploads/' . $module_name . '/images/' . $get_data['album_id'] . '/';
// tạo folder chứa ảnh theo album nếu chưa có:
$dir = '..' . $folder_name;
...
if ($nv_Request->isset_request('form_submit', 'post')) {
    $get_name = str_replace($folder_name, '', $form_data['path']);
    ...
    try {
        ...
        $query_insert->execute(); // nạp vào db
        $result = $query_insert->fetch(); // lấy dl vừa nạp
...
if($get_data['id'] && $nv_Request->isset_request('change_weight', 'get')) {
    ...
    if ($new_weight > 0) {
        $query_select = $query_builder->getBy(
            $tb_images,
            "album_id = " . $get_data['album_id'] . " AND id != " . $get_data['id'],
...
if ($nv_Request->isset_request('action_del', 'get')) {
    if ($get_data['id'] > 0 && md5($get_data['id'] . NV_CHECK_SESSION) == md5($get_data['checksess'])) {
        $image_del = $query_builder->getBy($tb_images, "album_id = " . $get_data['album_id'] . " AND id = " . $get_data['id'])->fetch();
        ...
        // cập nhật lại weight:
        try {
            $list_behind = $query_builder->getBy($tb_images, "album_id = " . $get_data['album_id'] . " AND weight > " . $image_del['weight'], ["order_by" => "weight ASC"])->fetchAll();
...
//=======[ show ]====================
// khai báo mặc định:
$album = $query_builder->getBy($tb_albums, "id = " . $get_data['album_id'])->fetch();

// phân trang
$perpage = 5;
$totals = $query_builder->countNumRows($tb_images, "album_id = " . $get_data['album_id']);
...
try {
    $query_select = $query_builder->getBy($tb_images, "album_id = " . $get_data['album_id'], ["order_by" => "weight ASC", "limit" => $perpage, "offset" => ($page - 1) * $perpage]);
...
$xtpl->assign('ALBUM_ID', $get_data['album_id']);
$xtpl->assign('OP', $op);
...
if (!empty($list_image) && count($list_image) > 0) {
    ...
    // phân trang:
    if ($perpage < $totals) {
        $paginate = nv_generate_page($url . "image_list", $totals, $perpage, $page);
