...
$page_title = $lang_module['slider_list'];
...
$folder_name = '/uploads/' . $module_name . '/sliders/' . $get_data['product_id'] . '/';
...
if ($nv_Request->isset_request('form_submit', 'post')) {
    ...
    $slider_name = time() . "-" . $helper->convertNameFile($get_name);

    try {
        $query_insert = $query_builder->insertSlider(
            $get_data['product_id'],
            $slider_name,
        ...
        $success = $lang_module['success_slider_create'];
...
if($get_data['id'] && $nv_Request->isset_request('change_weight', 'get')) {
    ...
    if ($new_weight > 0) {
        ...
        $weight = 0;
        while ($list_slider = $query_select->fetch()) {
            ++$weight;
            ...
            try {
                $query_update_weight = $query_builder->updateWeight($tb_sliders, $list_slider['id'], $weight);
        ...
        try {
            $query_update_new_weight = $query_builder->updateWeight($tb_sliders, $get_data['id'], $new_weight);
...
if($get_data['id'] && $nv_Request->isset_request('change_highlight', 'get')) {
    ...
    if ($new_status == 0 || $new_status == 1) {
        try {
            $query_update_active = $query_builder->updateHighlight($tb_sliders, $get_data['id'], $new_status);
...
if($get_data['id'] && $nv_Request->isset_request('change_active', 'get')) {
    ...
    if ($new_status == 0 || $new_status == 1) {
        try {
            $query_update_active = $query_builder->updateActive($tb_sliders, $get_data['id'], $new_status);
            $query_update_active->execute();
        } catch (\PDOException $ex) {
            print_r($ex->getMessage());
            die;
...
if ($nv_Request->isset_request('action_del', 'get')) {
    if ($get_data['id'] > 0 && md5($get_data['id'] . NV_CHECK_SESSION) == md5($get_data['checksess'])) {
        $slider_del = $query_builder->getBy($tb_sliders, "product_id = " . $get_data['product_id'] . " AND id = " . $get_data['id'])->fetch();

        // xoá:
        $query_builder->delslider($get_data['id']);

        // cập nhật lại weight:
        try {
            $list_behind = $query_builder->getBy($tb_sliders, "product_id = " . $get_data['product_id'] . " AND weight > " . $slider_del['weight'], ["order_by" => "weight ASC"])->fetchAll();
            $new_weight = $slider_del['weight'];
            foreach ($list_behind as $key => $behind) {
                $query_update_new_weight = $query_builder->updateWeight($tb_sliders, $behind['id'], $new_weight);
                $query_update_new_weight->execute();
                $new_weight ++;
            }
        } catch (\PDOException $ex) {
...
// phân trang
$perpage = 5;
$totals = $query_builder->countNumRows($tb_sliders, "product_id = " . $get_data['product_id']);
...
try {
    $query_select = $query_builder->getBy($tb_sliders, "product_id = " . $get_data['product_id'], ["order_by" => "weight ASC", "limit" => $perpage, "offset" => ($page - 1) * $perpage]);

    while ($slider = $query_select->fetch()) {
        $slider['highlight'] = ($slider['highlight'] == 1) ? 'checked' : '';
        $slider['active'] = ($slider['active'] == 1) ? 'checked' : '';
        $slider['created_at'] = date('d-m-Y\ H:i A', $slider['created_at']);
        $slider['updated_at'] = ($slider['updated_at'] != 0) ? date("d-m-Y\ H:i A", $slider['updated_at']) : '';

        $list_slider[$slider['id']] = $slider;
    }
} catch (\PDOException $ex) {
    print_r($ex->getMessage());
    die;
...
$xtpl->assign('MODULE_UPLOAD', $module_name . '/sliders'); // chỗ folder lưu ảnh đại diện
...
if (!empty($list_slider) && count($list_slider) > 0) {
    foreach ($list_slider as $key => $slider) {
        // cột stt:
        for ($i=1; $i <= $totals; $i++) {
            $xtpl->assign('STT', $i);
            $xtpl->assign('STT_SELECTED', ($i == $slider['weight']) ? "selected" : "");
    ...
    // phân trang:
    if ($perpage < $totals) {
        $paginate = nv_generate_page($base_url . "slider_list", $totals, $perpage, $page);
