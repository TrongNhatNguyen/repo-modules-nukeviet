...
<div class="well">
    <!-- BEGIN: product_name -->
    <table class="table">
        <caption><i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i> {LANG.prod_image_title} <em>{PROD.name}</em></caption>
    </table>
    <!-- END: product_name -->
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="w100 align-midlle">
                    <img src="{IMAGE.path}" alt="{IMAGE.name}" title="{LANG.zoom_in}" onclick="zoomImage('{IMAGE.path}', '{IMAGE.name}', '{LANG.zoom_out}');" style="width:100%;cursor: pointer;">
                </td>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("input[name=selectimg]").click(function() {
            ...
            var path = '{NV_UPLOADS_DIR}/{MODULE_UPLOAD}/{PROD_ID}'; //save image folder
    ...
    function change_weight(image_id) {
            let new_weight = $('.weight_' + image_id).val();
            window.location.replace(sPageURL + '&change_weight=true&id=' + image_id + '&new_weight=' + new_weight);
    }
    ...
    function change_highlight(image_id) {
        ...
        window.location.replace(sPageURL + '&change_highlight=true&id=' + image_id + '&new_status=' + new_status);
    }
    ...
    function change_active(image_id) {
        ...
        window.location.replace(sPageURL + '&change_active=true&id=' + image_id + '&new_status=' + new_status);
    }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Swal.fire(
                    ...
                    // xử lí:
                    window.location.replace(sPageURL + '&action_del=true&id=' + image_id + '&checksess=' + image_id + '{NV_CHECK_SESSION}');
                })
            }
        })
    }
